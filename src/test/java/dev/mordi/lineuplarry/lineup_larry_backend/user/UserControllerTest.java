...
public class UserControllerTest {
    ...
    void successfulCreationOfNewUser() throws Exception {
        ...
        try {
            ...
            assertThat(result.getResponse().getContentAsString())
                    .isEqualTo(expectedCreateUserJson);
    ...
    void declineUserCreationWithSuppliedId() throws Exception {
        ...
        InvalidUserException.IncludedUserIdException exception = new InvalidUserException.IncludedUserIdException(userIdToInvalidate);
        ...
        try {
            ...
            mockMvc.perform(post("/api/users")
                    ...
                    .andExpect(jsonPath("$.detail").value("Do not supply id when creating a user\nThe id '" + userIdToInvalidate + "' is invalid"))
                    .andReturn();
    ...
    void failToCreateEmptyUsernameForCreation() throws Exception {
        ...
        try {
            ...
            assertThat(res.getResponse().getContentAsString()).contains("username: username cannot be empty");
    ...
    void failToCreateBlankUsernameForCreation() throws Exception {
        ...
        try {
            ...
            assertThat(res.getResponse().getContentAsString()).contains("username: username cannot be blank");
            assertThat(res.getResponse().getContentType()).isEqualTo(MediaType.APPLICATION_PROBLEM_JSON.toString());
    ...
    void declineMismatchedUserIdOnUpdate() throws Exception {
        ...
        InvalidUserException.IdDoesNotMatchUserException exception = new InvalidUserException.IdDoesNotMatchUserException(nonExistentUserId, updatedUser);
    ...
    void failUpdateOnNullUsername() throws Exception {
        ...
        doThrow(exception).when(userService).updateUser(userWithNullUsername.id(), userWithNullUsername);
        try {
            String userWithNullUsernameJson = om.writeValueAsString(userWithNullUsername);

            var res = mockMvc.perform(put("/api/users/{id}", userWithNullUsername.id())
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(userWithNullUsernameJson))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.title").value("Invalid data")) // Should this be missing instead of null?
                    .andReturn();

            assertThat(res.getResponse().getContentAsString()).contains("username: username cannot be null");
            assertThat(res.getResponse().getContentType()).isEqualTo(MediaType.APPLICATION_PROBLEM_JSON.toString());
            verify(userService, never()).updateUser(userWithNullUsername.id(), userWithNullUsername);
        } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    void failUpdateOnEmptyUsername() throws Exception {
        ...
        doThrow(exception).when(userService).updateUser(userWithEmptyUsername.id(), userWithEmptyUsername);

        try {
            String userWithEmptyUsernameJson = om.writeValueAsString(userWithEmptyUsername);

            var res = mockMvc.perform(put("/api/users/{id}", userWithEmptyUsername.id())
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(userWithEmptyUsernameJson))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.title").value("Invalid data"))
            ...
            assertThat(res.getResponse().getContentAsString()).contains("username: username cannot be empty");
            assertThat(res.getResponse().getContentType()).isEqualTo(MediaType.APPLICATION_PROBLEM_JSON.toString());
            verify(userService, never()).updateUser(userWithEmptyUsername.id(), userWithEmptyUsername);
        } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    void failUpdateOnBlankUsername() throws Exception {
        ...
        InvalidUserException.BlankUsernameException exception = new InvalidUserException.BlankUsernameException();
        doThrow(exception).when(userService).updateUser(userWithBlankUsername.id(), userWithBlankUsername);

        try {
            String userWithBlankUsernameJson = om.writeValueAsString(userWithBlankUsername);

            var res = mockMvc.perform(put("/api/users/{id}", userId)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(userWithBlankUsernameJson))
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.title").value("Invalid data"))
            ...
            assertThat(res.getResponse().getContentAsString()).contains("username: username cannot be blank");
            assertThat(res.getResponse().getContentType()).isEqualTo(MediaType.APPLICATION_PROBLEM_JSON.toString());
            verify(userService, never()).updateUser(userWithBlankUsername.id(), userWithBlankUsername);
        } catch (JsonProcessingException e) {
            throw new RuntimeException(e);
        }
    }
