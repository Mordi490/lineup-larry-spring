...
public class UserControllerTest {
    ...
    void shouldReturnSomeUsers() throws Exception {
        when(userService.getAll()).thenReturn(
            List.of(new User(1L, "userOne"), new User(2L, "userTwo"))
        );
    ...
    void getFirstUser() throws Exception {
        ...
        var result = mockMvc
            .perform(get("/api/users/{id}", userIdToFetch))
            .andExpect(status().isOk())
    ...
    void getByIdOnNonexistentUser() throws Exception {
        ...
        when(userService.getById(nonExistentUserId)).thenReturn(
            Optional.empty()
        );

        mockMvc
            .perform(get("/api/users/{id}", nonExistentUserId))
    ...
    void declineUserCreationWithSuppliedId() throws Exception {
        ...
        InvalidUserException.IncludedUserIdException exception =
            new InvalidUserException.IncludedUserIdException(
                userIdToInvalidate
            );
    ...
    void failToCreateEmptyUsernameForCreation() throws Exception {
        ...
        InvalidUserException.EmptyUsernameException exception =
            new InvalidUserException.EmptyUsernameException();

        when(userService.createUser(userWithEmptyUsername)).thenThrow(
            exception
        );
    ...
    void failToCreateBlankUsernameForCreation() throws Exception {
        ...
        InvalidUserException.BlankUsernameException exception =
            new InvalidUserException.BlankUsernameException();

        when(userService.createUser(userWithBlankUsername)).thenThrow(
            exception
        );
    ...
    void declineMismatchedUserIdOnUpdate() throws Exception {
        ...
        InvalidUserException.IdDoesNotMatchUserException exception =
            new InvalidUserException.IdDoesNotMatchUserException(
                nonExistentUserId,
                updatedUser
            );
        doThrow(exception)
            .when(userService)
            .updateUser(nonExistentUserId, updatedUser);
        ...
        try {
            ...
            mockMvc
                .perform(
                    put("/api/users/{id}", nonExistentUserId)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void failUpdateOnNullUsername() throws Exception {
        ...
        InvalidUserException.NullUsernameException exception =
            new InvalidUserException.NullUsernameException();
        doThrow(exception)
            .when(userService)
            .updateUser(userWithNullUsername.id(), userWithNullUsername);
        try {
            String userWithNullUsernameJson = om.writeValueAsString(
                userWithNullUsername
            );

            var res = mockMvc
                .perform(
                    put("/api/users/{id}", userWithNullUsername.id())
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void failUpdateOnEmptyUsername() throws Exception {
        ...
        doThrow(exception)
            .when(userService)
            .updateUser(userWithEmptyUsername.id(), userWithEmptyUsername);
        ...
        try {
            ...
            var res = mockMvc
                .perform(
                    put("/api/users/{id}", userWithEmptyUsername.id())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(userWithEmptyUsernameJson)
                )
                .andExpect(status().isBadRequest())
    ...
    void failUpdateOnBlankUsername() throws Exception {
        ...
        doThrow(exception)
            .when(userService)
            .updateUser(userWithBlankUsername.id(), userWithBlankUsername);
    ...
    void successfulDeletion() throws Exception {
        ...
        mockMvc
            .perform(delete("/api/users/{id}", userIdToDelete))
