...
public class LineupControllerTest {
    ...
    void setUp() {
        ...
        this.lineupOne = new LineupWithAuthorDTO(1L, Agent.SOVA, Map.ASCENT, "lineup title",
                "lineup body", 1L, null, null, "John");
        this.lineupTwo = new LineupWithAuthorDTO(2L, Agent.SOVA, Map.ASCENT, "lineup title two",
                "lineup body two", 1L, null, null, "John");
        this.lineupWithSameTitleAsLineupOne = new LineupWithAuthorDTO(4L, Agent.SOVA, Map.ASCENT,
                "lineup title", "lineup body", 1L, null, null, "John");
    }
    ...
    void successfulGetAll() throws Exception {
        when(lineupService.getLineup(null, null, null, 20L, null))
                .thenReturn(Arrays.asList(lineupOne, lineupTwo));
        ...
        assertThat(result.getResponse().getContentType())
                .isEqualToIgnoringCase(MediaType.APPLICATION_JSON.toString());
    ...
    void getLineupsFromUserWithLineups() throws Exception {
        ...
        Optional<List<LineupWithAuthorDTO>> userOnesLineups = Optional
                .of(List.of(lineupOne, lineupTwo));
        when(lineupService.getAllLineupsFromUserId(userId, 20L, null))
    ...
    void getLineupsFromNonexistentUser() throws Exception {
        ...
        InvalidLineupException.NoUserException exception = new InvalidLineupException.NoUserException(
                nonexistentUserId);
        when(lineupService.getAllLineupsFromUserId(nonexistentUserId, 20L, null))
                .thenThrow(exception);
        ...
        mockMvc.perform(get("/api/lineups/user/{id}", nonexistentUserId))
                ...
                .andExpect(jsonPath("$.detail")
                        .value("No user with id: '" + nonexistentUserId + "' exists"));
    ...
    void successfulCreateLineup() throws Exception {
        Lineup lineupToCreate = new Lineup(null, Agent.SOVA, Map.ASCENT, "created title",
                "created body", userWithLineups.id(), null, null);
        Lineup expectedLineupCreatedResult = new Lineup(3L, Agent.SOVA, Map.ASCENT, "created title",
                "created body", userWithLineups.id(), null, null);
    ...
    void successfulCreateLineupWithNullId() throws Exception {
        Lineup lineupToCreate = new Lineup(null, Agent.SOVA, Map.ASCENT, "created title",
                "created body", userWithLineups.id(), null, null);
        Lineup expectedLineupResult = new Lineup(3L, Agent.SOVA, Map.ASCENT, "created title",
                "created body", userWithLineups.id(), null, null);
        when(lineupService.createLineup(lineupToCreate))
                .thenReturn(expectedLineupResult);
    ...
    void successfulUpdate() throws Exception {
        Lineup newLineupData = new Lineup(lineupOne.id(), Agent.SOVA, Map.ASCENT, "updated title",
                "updated body", lineupOne.userId(), null, null);
    ...
    void failUpdateOnBlankTitle() throws Exception {
        Lineup lineupWithBlankTitle = new Lineup(lineupOne.id(), lineupOne.agent(), lineupOne.map(),
                "  ", lineupOne.body(), lineupOne.userId(), null, null);

        try {
            String lineupWithBlankTitleJson = om.writeValueAsString(lineupWithBlankTitle);
            var res = mockMvc
                    .perform(put("/api/lineups/{id}", lineupWithBlankTitle.id())
                            .contentType(MediaType.APPLICATION_JSON)
            ...
            assertThat(res.getResponse().getContentAsString())
                    .contains("title: title cannot be blank");
            assertThat(res.getResponse().getContentType())
                    .isEqualTo(MediaType.APPLICATION_PROBLEM_JSON.toString());
            verify(lineupService, never()).updateLineup(lineupWithBlankTitle.id(),
    ...
    void failUpdateOnEmptyTitle() throws Exception {
        Lineup lineupWithEmptyTitle = new Lineup(lineupOne.id(), lineupOne.agent(), lineupOne.map(),
                "", lineupOne.body(), lineupOne.userId(), null, null);

        try {
            String lineupWithEmptyTitleJson = om.writeValueAsString(lineupWithEmptyTitle);
            var res = mockMvc
                    .perform(put("/api/lineups/{id}", lineupOne.id())
                            .contentType(MediaType.APPLICATION_JSON)
            ...
            assertThat(res.getResponse().getContentType())
                    .isEqualTo(MediaType.APPLICATION_PROBLEM_JSON.toString());
            verify(lineupService, never()).updateLineup(lineupWithEmptyTitle.id(),
    ...
    void failUpdateOnBlankBody() throws Exception {
        Lineup lineupWithEmptyTitle = new Lineup(lineupOne.id(), lineupOne.agent(), lineupOne.map(),
                lineupOne.title(), "   ", lineupOne.userId(), null, null);
    ...
    void failUpdateOnEmptyBody() throws Exception {
        Lineup lineupWithEmptyBody = new Lineup(lineupOne.id(), lineupOne.agent(), lineupOne.map(),
                lineupOne.title(), "", lineupOne.userId(), null, null);
        ...
        try {
            ...
            assertThat(res.getResponse().getContentAsString())
                    .contains("body: body cannot be empty");
            assertThat(res.getResponse().getContentType())
                    .isEqualTo(MediaType.APPLICATION_PROBLEM_JSON.toString());
            verify(lineupService, never()).updateLineup(lineupWithEmptyBody.id(),
                    lineupWithEmptyBody);
        } catch (JacksonException e) {
            throw new RuntimeException(e);
        }
    ...
    void successfulGetByTitle() {
        List<LineupWithAuthorDTO> mockedResult = Arrays.asList(lineupOne,
                lineupWithSameTitleAsLineupOne);
        when(lineupService.getByTitle("lineup title", 20L, null))
    ...
    void successfulGetByTitleNoMatches() {
        ...
        when(lineupService.getByTitle("bad search title", 20L, null))
                .thenReturn(mockedResult);
