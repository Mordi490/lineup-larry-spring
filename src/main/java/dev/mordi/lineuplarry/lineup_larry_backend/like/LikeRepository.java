...
public class LikeRepository {
  ...
  public Optional<Like> getLikeById(Long lineupId, Long userId) {
    return dsl.select(LIKES.LINEUP_ID, LIKES.USER_ID, LIKES.CREATED_AT)
        ...
        .and(LIKES.LINEUP_ID.eq(lineupId))
        .fetchOptional()
  ...
  public Like likeLineup(Like like) {
    ...
    boolean lineupExistence =
        dsl.fetchExists(selectFrom(LINEUP).where(LINEUP.ID.eq(like.lineupId())));
    ...
    // going to assume that the like.userId cannot be invalid, double check after auth has been
    // added
  ...
  public void removeLike(Like like) {
    boolean exists =
        dsl.fetchExists(
            selectFrom(LIKES)
                .where(LIKES.USER_ID.eq(like.userId()))
    ...
    dsl.deleteFrom(LIKES)
        .where(LIKES.LINEUP_ID.eq(like.lineupId()))
        .and(LIKES.USER_ID.eq(like.userId()))
        .execute();
  }
  ...
  public long getLikeCountByLineup(Long lineupId) {
    ...
    Long count =
        dsl.selectCount().from(LIKES).where(LIKES.LINEUP_ID.eq(lineupId)).fetchOne(0, long.class);
