...
public class LineupController {
  ...
  public ResponseEntity<List<LineupWithAuthorDTO>> getLineups(
    ...
    List<LineupWithAuthorDTO> lineups =
        lineupService.getLineup(title, agent, map, pageSize, lastValue.orElse(null));
  ...
  public ResponseEntity<LineupWithAuthorDTO> getById(@PathVariable Long id) {
    ...
    return lineup.map(ResponseEntity::ok).orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
  }
  ...
  public ResponseEntity<List<LineupWithAuthorDTO>> getAllLineupsFromUser(
    ...
    Optional<List<LineupWithAuthorDTO>> lineups =
        lineupService.getAllLineupsFromUserId(id, pageSize, lastValue.orElse(null));

    return lineups.map(ResponseEntity::ok).orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
  }
}
