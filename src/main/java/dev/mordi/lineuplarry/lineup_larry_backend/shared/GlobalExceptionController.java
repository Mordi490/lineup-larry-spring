...
public class GlobalExceptionController extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleTypeMismatch(
            ...
            WebRequest request) {
        String parameterName = e instanceof MethodArgumentTypeMismatchException mismatch
                ? mismatch.getName()
                : "parameter";
        ...
        String detail = "Invalid value '%s' for parameter '%s'. Expected type: '%s'"
                .formatted(e.getValue(), parameterName, expectedType);

        ProblemDetail problemDetail = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST,
                detail);
        problemDetail.setTitle("Invalid data");
        problemDetail
                .setType(URI.create("https://lineup-larry.dev/problems/request/invalid-parameter"));
    ...
    protected ResponseEntity<Object> handleHandlerMethodValidationException(
            HandlerMethodValidationException e, HttpHeaders headers, HttpStatusCode status,
            WebRequest request) {
        ProblemDetail problemDetail = ProblemDetail.forStatusAndDetail(HttpStatus.BAD_REQUEST,
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException e,
        ...
        if (isInvalidFormatException(cause)) {
            ...
            if (targetType == Agent.class) {
                return toProblemResponse(
                        new InvalidLineupException.InvalidAgentException(invalidValue), headers);
            }
            if (targetType == Map.class) {
                return toProblemResponse(
                        new InvalidLineupException.InvalidMapException(invalidValue), headers);
            }
        }
        ...
        problemDetail.setTitle("Bad Request");
        problemDetail
                .setType(URI.create("https://lineup-larry.dev/problems/request/unreadable-body"));
    ...
    private static String buildHandlerValidationDetail(HandlerMethodValidationException e) {
        ...
        e.getParameterValidationResults().forEach(result -> {
            if (result instanceof ParameterErrors parameterErrors) {
                parameterErrors.getFieldErrors().forEach(fieldError -> messages
                        .add(fieldError.getField() + ": " + fieldError.getDefaultMessage()));
                return;
            }
            result.getResolvableErrors().forEach(resolvableError -> {
    ...
    private static boolean isInvalidFormatException(Throwable cause) {
        return (cause != null && cause.getClass().getSimpleName().equals("InvalidFormatException"));
    }

    private static Class<?> getTargetType(Throwable cause) {
        try {
            Object result = cause.getClass().getMethod("getTargetType").invoke(cause);
